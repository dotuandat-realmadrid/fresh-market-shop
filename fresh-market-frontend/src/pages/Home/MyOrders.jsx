  ...
  const Sidebar = () => (
    <div className="account-sidebar">
      ...
      <ul className="account-sidebar-menu">
        ...
        <li>
          <Link to="/my-address">Danh sách địa chỉ</Link>
        </li>
        <li>
          <Link to="/my-orders" className="active">
            Đơn hàng của tôi
          </Link>
        </li>
      </ul>
      ...
      <Row align="middle">
        <Col xs={24} md={5} className="order-info-section">
          ...
          <div className="customer-mini-info">
            <div>
              <UserOutlined /> {order.fullName || user.fullName}
            </div>
            <div>
              <PhoneOutlined /> {order.phone || user.phone}
            </div>
          </div>
        </Col>
              ...
              <div key={idx} className="order-item-row-compact">
                ...
                <div className="compact-details">
                  ...
                  <div className="compact-info-row">
                    <span className="p-code text-muted small">
                      Mã: {item.productCode || item.sku || "N/A"}
                    </span>
                  </div>
                  <div className="compact-price-qty">
                    <span className="price">
                      {(item.priceAtPurchase || item.price || 0).toLocaleString(
                        "vi-VN",
                      )}
                      đ
                    </span>
          ...
          <div className="action-buttons" onClick={(e) => e.stopPropagation()}>
            {order.status === "PENDING" && (
              <>
                ...
                >
                  <Button danger size="small" className="btn-action-custom">
                    Hủy đơn hàng
                  </Button>
                </Popconfirm>
                {order.paymentMethod && order.paymentMethod !== "COD" && (
                  <Button
                    className="btn-refund-custom"
                    size="small"
    ...
    <div className="address-list-container">
            ...
            <Tabs
              ...
              items={statuses.map((s) => {
                ...
                if (activeStatus === s.key) {
                  if (s.key === "CANCELLED" || s.key === "FAILED")
                    color = "#ff4d4f";
                ...
                return {
                  ...
                  label: (
                    <span
                      style={{
                        color: color,
                        fontWeight: activeStatus === s.key ? "600" : "normal",
                      }}
                    >
                      {s.label}
                    </span>
        ...
        {selectedOrder && (
          <div className="modern-detail-wrapper">
                ...
                items={[
                  {
                    ...
                    description: (
                      <span style={{ color: "#555" }}>Đặt hàng thành công</span>
                    ),
                    icon:
                      (selectedOrder.status === "CANCELLED" ||
                        selectedOrder.status === "FAILED") &&
                      getStatusStep(selectedOrder.status) === 0 ? (
                        <CloseCircleFilled
              ...
              <Col span={12}>
                <div className="info-block-styled">
                  ...
                  <div className="block-body-styled">
                    <div className="body-row-styled">
                      <span className="label-styled">Họ và tên:</span>{" "}
                      <span className="val-styled">
                        {selectedOrder.fullName}
                      </span>
                    </div>
                    <div className="body-row-styled">
                      <span className="label-styled">Email:</span>{" "}
                      <span className="val-styled">
                        {selectedOrder.username}
                      </span>
                    </div>
                    <div className="body-row-styled">
                      <span className="label-styled">Số điện thoại:</span>{" "}
                      <span className="val-styled">{selectedOrder.phone}</span>
                    </div>
                    <div className="body-row-styled">
                      <span className="label-styled">Địa chỉ:</span>{" "}
                      <span className="val-styled">
              ...
              <Col span={12}>
                <div className="info-block-styled">
                  ...
                  <div className="block-body-styled">
                    ...
                    <div className="body-row-styled">
                      <span className="label-styled">Tổng tiền:</span>{" "}
                      <span className="val-styled red bold">
                        {selectedOrder.totalPrice.toLocaleString()}đ
                      </span>
                    </div>
                    <div className="body-row-styled">
                      <span className="label-styled">Ghi chú:</span>{" "}
                      <span className="val-styled">
                        {selectedOrder.note || "Không có"}
                      </span>
                    </div>
                    <div className="body-row-styled">
                      <span className="label-styled">Ngày tạo:</span>{" "}
                      <span className="val-styled">
            ...
            <div className="product-table-modern">
              ...
              <Table
                dataSource={
                  selectedOrder.orderItems || selectedOrder.details || []
                }
                pagination={false}
                size="middle"
                rowKey={(record) =>
                  record.productId || record.sku || record.id || Math.random()
                }
                columns={[
                  {
                    title: "Tên sản phẩm",
                    render: (_, record) => (
                      <div className="table-product-item">
                        ...
                        <div className="p-text">
                          ...
                          <div className="p-code-small">
                            Mã: {record.sku || record.productCode}
                          </div>
                        </div>
                      </div>
                    ),
                  },
                  {
                    ...
                    dataIndex: "quantity",
                    align: "center",
                    render: (q) => <div className="qty-badge-square">{q}</div>,
                  },
                  {
                    title: "Giá",
                    render: (_, r) =>
                      `${(r.price || r.priceAtPurchase).toLocaleString()}đ`,
                    align: "right",
                  },
                  {
                    title: "Thành tiền",
                    render: (_, r) => (
                      <span className="red-bold">
                        {(
                          (r.price || r.priceAtPurchase) * r.quantity
                        ).toLocaleString()}
                        đ
                      </span>
                    ),
                    ...
                    render: (_, r) =>
                      selectedOrder.status === "COMPLETED" ? (
                        r.isReviewed ? (
                          ...
                          <Button
                            className="btn-green-eval"
                            type="primary"
                            size="middle"
                            onClick={() => handleOpenReview(r, selectedOrder)}
                          >
                            Đánh giá
                          </Button>
                        )
              ...
              <Space size="middle">
                {selectedOrder.status === "PENDING" && (
                  <Popconfirm
                    ...
                    cancelText="Hủy"
                  >
                    <Button danger size="middle" className="btn-cancel-outline">
                      Hủy đơn hàng
                    </Button>
                  </Popconfirm>
                )}
      ...
      >
        <div className="form-group mb-3">
          ...
          <Select
            value={refundData.code}
            onChange={(v) => setRefundData({ ...refundData, code: v })}
            style={{ width: "100%" }}
          >
        ...
        <div className="form-group mb-3">
          <label className="bold mb-1 d-block text-start">
            Lý do hoàn tiền
          </label>
          <Radio.Group
          ...
          {refundData.reason === "Khác" && (
            <TextArea
              ...
              value={refundData.otherReason}
              onChange={(e) =>
                setRefundData({ ...refundData, otherReason: e.target.value })
              }
            />
          )}
        </div>
            ...
            <div className="p-details">
              ...
              <div className="p-price">
                ...
                <span className="red-text">
                  {reviewData.price.toLocaleString()}đ
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="stars-section text-center">
            ...
            <div className="stars-label">
              {reviewData.rating === 0
                ...
                : reviewData.rating === 1 || reviewData.rating === 2
                  ? "Không hài lòng"
                  : reviewData.rating === 3
                    ? "Bình thường"
                    : reviewData.rating === 4
